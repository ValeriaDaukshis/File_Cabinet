...
namespace FileCabinetApp.CommandHandlers
{
    ...
    public class InputValidator
    {
        ...
        public FileCabinetRecord CheckInputFields(string[] fields, string[] values, FileCabinetRecord record)
        {
            ...
            var firstName = ReadInput(fields, values, record.FirstName, FirstNameSearcher, this.converter.StringConverter, FirstNameValidator);
            var lastName = ReadInput(fields, values, record.LastName, LastNameSearcher, this.converter.StringConverter, LastNameValidator);
            var gender = ReadInput(fields, values, record.Gender, GenderSearcher, this.converter.CharConverter, GenderValidator);
            var dateOfBirth = ReadInput(fields, values, record.DateOfBirth, DateOfBirthSearcher, this.converter.DateTimeConverter, DateOfBirthValidator);
            var credit = ReadInput(fields, values, record.CreditSum, CreditSumSearcher, this.converter.DecimalConverter, CreditSumValidator);
            var duration = ReadInput(fields, values, record.Duration, DurationSearcher, this.converter.ShortConverter, DurationValidator);
        ...
        public FileCabinetRecord PrintInputFields(ModelWriters modelWriter)
        {
            ...
            var firstName = ReadInput(this.converter.StringConverter, FirstNameValidator, modelWriter);
            modelWriter.Writer.Invoke("Last name: ");
            var lastName = ReadInput(this.converter.StringConverter, LastNameValidator, modelWriter);
            modelWriter.Writer.Invoke("Gender(M/F): ");
            var gender = ReadInput(this.converter.CharConverter, GenderValidator, modelWriter);
            modelWriter.Writer.Invoke("Date of birth(mm/dd/yyyy): ");
            var dateOfBirth = ReadInput(this.converter.DateTimeConverter, DateOfBirthValidator, modelWriter);
            modelWriter.Writer.Invoke("Credit sum(bel rub): ");
            var credit = ReadInput(this.converter.DecimalConverter, CreditSumValidator, modelWriter);
            modelWriter.Writer.Invoke("Credit duration(month): ");
            var duration = ReadInput(this.converter.ShortConverter, DurationValidator, modelWriter);
            return new FileCabinetRecord(firstName, lastName, gender, dateOfBirth, credit, duration);
        }
        ...
        {
            var firstName = ReadInput(fields, values, FirstNameSearcher, this.converter.StringConverter, FirstNameValidator);
            var lastName = ReadInput(fields, values, LastNameSearcher, this.converter.StringConverter, LastNameValidator);
            var gender = ReadInput(fields, values, GenderSearcher, this.converter.CharConverter, GenderValidator);
            var dateOfBirth = ReadInput(fields, values, DateOfBirthSearcher, this.converter.DateTimeConverter, DateOfBirthValidator);
            var credit = ReadInput(fields, values, CreditSumSearcher, this.converter.DecimalConverter, CreditSumValidator);
            var duration = ReadInput(fields, values, DurationSearcher, this.converter.ShortConverter, DurationValidator);
            return new FileCabinetRecord(firstName, lastName, gender, dateOfBirth, credit, duration);
        }
        ...
        private static T ReadInput<T>(
            ...
            string[] values,
            T record,
            Func<string[], string[], Tuple<bool, string>> finder,
            Func<string, Tuple<bool, string, T>> converter,
            Func<T, Tuple<bool, string>> validator)
        {
            if (finder is null)
            {
                throw new ArgumentNullException(nameof(finder), $"{nameof(finder)} is null");
